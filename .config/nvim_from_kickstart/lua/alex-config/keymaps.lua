...
M.whichkey_spec = {
    ...
    { "<leader>q",  vim.diagnostic.setloclist, desc = "Open diagnostics [Q]uickfix list", nowait = true, remap = false },
...
M.set_copilot_keys = function()
    keymap("i", "<C-L>", "copilot#Accept('<CR>')", nil,
        { noremap = true, silent = true, expr = true, replace_keycodes = false })
    keymap('i', '<C-S>', '<Plug>(copilot-accept-word)')
end
...
M.set_oil_keys = function()
    ...
    keymap("n", "<leader>nh", "<cmd>vertical leftabove split<CR><cmd>Oil<CR>", "Split Left",
        { nowait = true, remap = false })
    keymap("n", "<leader>nj", "<cmd>horizontal belowright split<CR><cmd>Oil<CR>", "Split Down",
        { nowait = true, remap = false })
    keymap("n", "<leader>nk", "<cmd>horizontal leftabove split<CR><cmd>Oil<CR>", "Split Up",
        { nowait = true, remap = false })
    keymap("n", "<leader>nl", "<cmd>vertical belowright split<CR><cmd>Oil<CR>", "Split Right",
        { nowait = true, remap = false })
end

M.set_harpoon_keys = function(harpoon)

    keymap("n", "<leader>a", function() harpoon:list():add() end, "[A]dd file to Harpoon")
    keymap("n", "<leader>e", function() harpoon.ui:toggle_quick_menu(harpoon:list()) end, "[E]xplore Harpoon")

    keymap("n", "<C-h>", function() harpoon:list():select(1) end, "[H]arpoon [1]")
    keymap("n", "<C-t>", function() harpoon:list():select(2) end, "[H]arpoon [2]")
    keymap("n", "<C-n>", function() harpoon:list():select(3) end, "[H]arpoon [3]")
    keymap("n", "<C-s>", function() harpoon:list():select(4) end, "[H]arpoon [4]")

    keymap("n", "<C-S-P>", function() harpoon:list():prev() end, "[H]arpoon [P]revious")
    keymap("n", "<C-S-N>", function() harpoon:list():next() end, "[H]arpoon [N]ext")
end
